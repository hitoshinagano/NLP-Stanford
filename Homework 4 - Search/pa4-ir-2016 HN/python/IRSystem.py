...
class IRSystem:
    ...
    def compute_tfidf(self):
        ...
        for word in self.vocab:
            for title in self.titles:
                if word not in self.tfidf:
                    self.tfidf[word] = {}
                self.tfidf[word][title] = 0.0

        # First calculate idf, a dictionary
        # idf['word'] = word's idf value
        N = float( len(self.docs) )
        idf = { word : math.log10( N/len(self.get_posting(word)) ) for word in self.vocab }

        # calculate tf based on length of the length of inverted index : len(inv_index[word][title])
        # then calculate tfidf as multiplication of tf and idf
        # if tf == 0 its tf-idf value will remain 0.0 (initialized above)
        for word in self.vocab:
            for title in self.titles:
                tf = len(self.inv_index[word][title])
                if tf != 0:
                    self.tfidf[word][title] = ( 1 + math.log10(tf) ) * idf[word]

        # compute L2 norm for each document
        self.tfidf_L2_norm = {}
        for title_id, title in enumerate(self.titles):
            L2_norm_squared = 0.0
            for word in self.docs[title_id]:
                L2_norm_squared += self.tfidf[word][title] ** 2
            self.tfidf_L2_norm[title] = math.sqrt(L2_norm_squared)
    ...
    def get_tfidf(self, word, document):
        ...
        #       document index.
        doc_title = self.get_title(document)
        tfidf = self.tfidf[word][doc_title]
        # ------------------------------------------------------------------
        return tfidf
    ...
    def get_posting(self, word):
        ...
        posting = [ i for i, title in enumerate(self.titles) if len(self.inv_index[word][title]) != 0 ]
    ...
    def boolean_retrieve(self, query):
        ...
        docs = []
        a_set = set(self.get_posting(query[0]))
        for query_word in query[1:]:
            a_set = a_set.intersection(self.get_posting(query_word))
    ...
    # get_title returns the title name given the title id
    # had to make this function because tfidf dictionary uses title instead of a title_id
    def get_title(self, title_i):
        return self.titles[title_i]
    ...
    def phrase_retrieve(self, query):
        ...
        # iterates over selected titles based on query
        for candidate_title_i in self.boolean_retrieve(query):
            candidate_title = self.get_title(candidate_title_i)

            # builds a q_list of inverted indexes for each query word
            q_list = []
            for q_word in query:
                q_list.append(self.inv_index[q_word][candidate_title])

            # corner case when q_list has only one list (eq. to query for only one word)
            if len(q_list) == 1:
                docs.append(candidate_title_i)
                break

            # if query has multiple words, need to check if word indices show up in sequence
            # among all lists
            query_found = False
            for i in q_list[0]:
    ...
    def rank_retrieve(self, query):
        ...
        # tf_query is the tf for each word in query
        # query.count(word) gives the counts of word in query
        # query_L2_norm is the L2 norm of the query tf vector
        tf_query = {word: 1 + math.log10(query.count(word)) for word in words_in_query}

        # nao precisa !!!!! o PA pede ltc.lnn (lnn significa que o query nao tem normalizacao, nem idf)
        query_L2_norm = 0.0
        for word in words_in_query:
            query_L2_norm += tf_query[word] ** 2
        query_L2_norm = math.sqrt(query_L2_norm)

        query_L2_norm = 1.0

        for title_id in range(len(self.titles)):
            score = 0.0
            for word in words_in_query:
                score += tf_query[word] * self.tfidf[word][self.get_title(title_id)]
            score = score / (query_L2_norm * self.tfidf_L2_norm[self.get_title(title_id)])
            scores[title_id] = score
