...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for i in xrange(1, len(word) + 1):
      ...
      # The correct signal is the character followed by an inserted letter for all letters
      for l in self.ALPHABET:
        correctLetters = corruptLetters + l
        correction = "%s%s%s" % (word[1:i], l, word[i:])
        ret.append(Edit(correction, corruptLetters, correctLetters))
  ...
  def transposeEdits(self, word):
    ...
    for i in xrange(0, len(word) - 1):
      # The corrupted signal is this character and the next
      corruptLetters = word[i:i+2]
      # The correct signal is the next character swaped by the current character
  ...
  def replaceEdits(self, word):
    ...
    for i in xrange(0, len(word)):
      ...
      for l in set(self.ALPHABET) - {corruptLetters}:
        correctLetters = l
        correction = "%s%s%s" % (word[:i], l, word[i+1:])
        ret.append(Edit(correction, corruptLetters, correctLetters))
  ...
  def editProbabilities(self, misspelling):
    ...
    wordTotal  = 0      # edits(misspelling) eh montado para cada palavra no runtime
    for edit in self.edits(misspelling):                                           # editCounts eh um dicionario
                                                                                   # com conteudo de count_1edit.txt
